...
export function UpcomingPage() {
  const { upcoming, recent, allDetails, isLoading } = useMatchSchedule()
  ...
  const filteredUpcoming = leagueFilter === 'all'
    ? upcoming
    : upcoming.filter((e) => e.league.slug === leagueFilter)

  const filteredRecent = leagueFilter === 'all'
    ? recent
    : recent.filter((e) => e.league.slug === leagueFilter)
          ...
          <div className="bg-surface-elevated border-2 border-t-0 border-surface-muted overflow-hidden">
            ...
            <ul className="divide-y divide-surface-muted">
              {filteredRecent.map((event) => (
                <RecentRow key={event.match.id} event={event} findSlug={findSlug} details={allDetails.get(event.match.id)} />
              ))}
            </ul>
          </div>
        </div>
          ...
          {filteredUpcoming.length === 0 ? (
            ...
            <ul className="divide-y divide-surface-muted">
              {filteredUpcoming.map((event) => (
                <UpcomingRow key={event.match.id} event={event} findSlug={findSlug} details={allDetails.get(event.match.id)} />
              ))}
            </ul>
          )}
        </div>
